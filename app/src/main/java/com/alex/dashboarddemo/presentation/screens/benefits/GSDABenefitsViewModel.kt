...
@HiltViewModel
class GSDABenefitsViewModel @Inject constructor(
    private val useCases: GSDAUseCases,
) : GSDADashboardViewModel(useCases) {
    /*private fun getInfo(key: String) {
        viewModelScope.launch(Dispatchers.IO) {
            useCases.getRemoteConfigUseCase.invoke(key).collect { benefitsData ->
                when (benefitsData) {
                    ...
                    is GSDAResult.Success -> {
                        setState { copy(getInfo = OnSuccess(benefitsData.data)) }
                    }
