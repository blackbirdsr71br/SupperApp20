...
@HiltViewModel
class GSDAExploreViewModel @Inject constructor(
    private val useCases: GSDAUseCases,
) : GSDADashboardViewModel(useCases) {
    /*private fun getConfig(key: String) {
        viewModelScope.launch(Dispatchers.IO) {
            useCases.getRemoteConfigUseCase.invoke(key).collect { exploreData ->
                when (exploreData) {
                    ...
                    is GSDAResult.Success -> {
                        setState { copy(getInfo = OnSuccess(exploreData.data)) }
                    }
